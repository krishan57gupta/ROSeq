...
exe_time=c()
pred_label_founder<-function(L,file,i,j,ii)
{
  ...
  saveRDS(all_time,paste("~/ROSeq/paper_work/latest_data/noise_jurkat/time/",file,"_",i,"_",j,"_",ii,".rds",sep=""))
  R=cbind(R4$df,
          R5$df,
          R6$df,
          R10,
...
{
  if(file.exists(paste("~/ROSeq/paper_work/latest_data/noise_jurkat/null_noise_uniform_2_10","_",k,".rds",sep="")))
  { 
    next
   }else
    {
      ...
      for (ii in c(1,2,3,4))
      {
          ...
          if(random_subgroup==1)
          {
            ...
            for(i in 1:dim(F_D)[1])
            {
              ...
              random_10_percent_index=sample(1:length(F_D[i,]),as.integer(length(F_D[i,])/10))
              random_10_percent_sample=sample(c(1:as.integer(mean(F_D[i,])*ii/10)),as.integer(length(F_D[i,])/10),replace = TRUE)
          ...
          if(random_subgroup==2)
          {
            ...
            for(i in 1:dim(S_D)[1])
            {
              ...
              random_10_percent_index=sample(1:length(S_D[i,]),as.integer(length(S_D[i,])/10))
              random_10_percent_sample=sample(c(1:as.integer(mean(S_D[i,])*ii/10)),as.integer(length(S_D[i,])/10),replace = TRUE)
              zero_samples[random_10_percent_index]<-random_10_percent_sample
          ...
          D_E_2=pred_label_founder(L,"null_noise_uniform_2_10","gene",k,ii)
          ...
          L$count=cbind(F_D,S_D)
          L$condt=c(rep(1,dim(F_D)[2]),rep(2,dim(S_D)[2]))
          D_E_2=pred_label_founder(L,"null_noise_uniform_2_10","sample",k,ii)
          result$sample=append(result$sample,list(D_E_2))
          saveRDS(result,paste("~/ROSeq/paper_work/latest_data/noise_jurkat/null_noise_uniform_2_10","_",k,".rds",sep=""))
        }
  }
}
