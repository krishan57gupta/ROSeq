...
{
  if(file.exists(paste("~/ROSeq/paper_work/latest_data/noise_jurkat/null_noise_2_10","_",k,".rds",sep="")))
  { 
    next
   }
  else
        ...
        for (jj in c(.2,.4))
        {
          ...
          if(random_subgroup==1)
          {
            ...
            for(i in 1:dim(F_D)[1])
            {
              ...
              random_10_percent_sample=rnorm(as.integer(length(F_D[i,])/10),mean=(mean(F_D[i,])+sd(F_D[i,])*ii),sd=sd(F_D[i,])*jj)
          ...
          if(random_subgroup==2)
          {
            ...
            for(i in 1:dim(S_D)[1])
            {
              ...
              random_10_percent_sample=rnorm(as.integer(length(S_D[i,])/10),mean=(mean(S_D[i,])+sd(S_D[i,])*ii),sd=sd(S_D[i,])*jj)
              zero_samples[random_10_percent_index]<-random_10_percent_sample
          ...
          D_E_2=pred_label_founder(L,"null_noise_2_10","gene",k,ii,jj)
          ...
          L$count=cbind(F_D,S_D)
          L$condt=c(rep(1,dim(F_D)[2]),rep(2,dim(S_D)[2]))
          D_E_2=pred_label_founder(L,"null_noise_2_10","sample",k,ii,jj)
          result$sample=append(result$sample,list(D_E_2))
          saveRDS(result,paste("~/ROSeq/paper_work/latest_data/noise_jurkat/null_noise_2_10","_",k,".rds",sep=""))
        }
      }
  }
}
