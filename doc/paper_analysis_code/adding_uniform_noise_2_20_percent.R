...
{
  if(file.exists(paste("~/ROSeq/paper_work/latest_data/noise_jurkat/null_noise_uniform_2_20","_",k,".rds",sep="")))
  { 
    next
   }
  else
      ...
      for (ii in c(1,2,3,4))
      {
          ...
          if(random_subgroup==1)
          {
            ...
            for(i in 1:dim(F_D)[1])
            {
              ...
              random_10_percent_sample=sample(c(1:as.integer(mean(F_D[i,])*ii/10)),as.integer(length(F_D[i,])/5),replace = TRUE)
          ...
          if(random_subgroup==2)
          {
            ...
            for(i in 1:dim(S_D)[1])
            {
              ...
              random_10_percent_sample=sample(c(1:as.integer(mean(S_D[i,])*ii/10)),as.integer(length(S_D[i,])/5),replace = TRUE)
              zero_samples[random_10_percent_index]<-random_10_percent_sample
          ...
          D_E_2=pred_label_founder(L,"null_noise_uniform_2_20","gene",k,ii)
          ...
          L$count=cbind(F_D,S_D)
          L$condt=c(rep(1,dim(F_D)[2]),rep(2,dim(S_D)[2]))
          D_E_2=pred_label_founder(L,"null_noise_uniform_2_20","sample",k,ii)
          result$sample=append(result$sample,list(D_E_2))
          saveRDS(result,paste("~/ROSeq/paper_work/latest_data/noise_jurkat/null_noise_uniform_2_20","_",k,".rds",sep=""))
        }
  }
}
