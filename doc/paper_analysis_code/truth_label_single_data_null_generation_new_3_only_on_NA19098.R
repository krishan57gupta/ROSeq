...
pred_label_founder<-function(L,file,i,j)
{
  ...
  D_time <- Sys.time()
  R7=ROSeq(countData = Linnorm(L$count), condition = L$condt, numCores = 1)[,c(1,2)]
  E_time <- Sys.time()
  R8=ROSeq(countData = edgeR::cpm(L$count), condition = L$condt, numCores = 1)[,c(1,2)]
  F_time <- Sys.time()
  R9=ROSeq(countData = TMMnormalization(L$count), condition = L$condt, numCores = 1)[,c(1,2)]
  G_time <- Sys.time()
  R10=ROSeq(countData = limma::voom(TMMnormalization(L$count))$E, condition = L$condt, numCores = 1)[,c(1,2)]
  H_time <- Sys.time()
  R11=ROSeq(countData = median_normalization(L$count), condition = L$condt, numCores = 1)[,c(1,2)]
  I_time <- Sys.time()
  R12=DESeq2(L)
  ...
  names(all_time)=c("MAST",
                    ...
                    "DEseq2",
                    "SCDE")
  saveRDS(all_time,paste("~/ROSeq/paper_work/latest_data/NA19098/time/",file,"_",j,"_",i,".rds",sep=""))
...
L_Tung$NA19098_NA19098_pred_label=list()
for (j in c(10,25,50,75,100,125,150))
{
  if(file.exists(paste("~/ROSeq/paper_work/latest_data/NA19098/L_Tung_single_null_NA19098","_",j,".rds",sep="")))
  { 
    temp_object<-readRDS(paste("~/ROSeq/paper_work/latest_data/NA19098/L_Tung_single_null_NA19098","_",j,".rds",sep=""))
    L_Tung$NA19098_NA19098_pred_label<-temp_object$NA19098_NA19098_pred_label
    }
  else
  { 
     L_Tung$NA19098_NA19098_pred_label=list()
     }
  if(length(L_Tung$NA19098_NA19098_pred_label)>=100)
  {
    next
    }
  for(i in (length(L_Tung$NA19098_NA19098_pred_label)+1):100)
  {
    print(paste(j,i,sep="_"))
  rand_samp=sample(1:dim(L_Tung$NA19098_count)[2],j)
  rand_samp_1=sample(1:dim(L_Tung$NA19098_count)[2],j)
  L_Tung$NA19098_NA19098_count=cbind(L_Tung$NA19098_count[,c(rand_samp)],L_Tung$NA19098_count[,c(rand_samp_1)])
  L_Tung$NA19098_NA19098_group=c(rep(1,length(L_Tung$NA19098_group[c(rand_samp)])),rep(2,length(L_Tung$NA19098_group[c(rand_samp_1)])))
  
  L=list()
  L$count=L_Tung$NA19098_NA19098_count
  L$condt=L_Tung$NA19098_NA19098_group
  L_Tung$NA19098_NA19098_pred_label[[i]]=pred_label_founder(L,"null_NA19098_time",i,j)
  saveRDS(L_Tung,paste("~/ROSeq/paper_work/latest_data/NA19098/L_Tung_single_null_NA19098","_",j,".rds",sep=""))
  }
}
