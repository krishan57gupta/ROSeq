file="~/ROSeq/paper_work/latest_data/noise_jurkat/null_noise_2_10_"
...
for (k in c(1:6))
{
  ...
  rownames(mat)=paste(c("m+2s, ","m+2s, ","m+4s, ","m+4s, "),c("0.2s","0.4s"),sep="")
...
pdf("~/ROSeq/paper_work/latest_plot/noise_jurkat/gaussion_noise_combined_all_gene_2_10_pval.pdf",height = 10)
a
dev.off()
a
# ######################################## another way
...
a=ggarrange(plot[[1]],plot[[2]],plot[[3]],plot[[4]],plot[[5]],plot[[6]],
            ncol = 1,nrow=6,common.legend = TRUE)
pdf("~/ROSeq/paper_work/latest_plot/noise_jurkat/gaussion_noise_combined_all_sample_2_10_pval.pdf",height = 10)
a
dev.off()
a
